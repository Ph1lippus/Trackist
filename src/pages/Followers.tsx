...
const FollowersPage = () => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                const followersData = await getCachedOrFetch(
                    'followers',
                    targetUserId,
                    async () => (await getFollowersList(targetUserId)).data || [],
                    { ttl: 2 * 60 * 1000, staleWhileRevalidate: true }
                )
                if (followersData && active) {
    ...
    const followingSet = useMemo(() => {
        ...
        return new Set(followers.filter(f => currentUser.id !== f.id).map(f => f.id))
    ...
    const handleFollow = useCallback(async (userId: string) => {
        ...
        if (isCurrentlyFollowing) {
            ...
            await cacheService.clearPattern('followers')
            ...
            <div className="friends-container">
                ...
                {isLoading ? (
                    ...
                    <div className="discover-empty">
                        ...
                        <p>{targetUser ? `${displayName} doesn't have any followers yet.` : "You don't have any followers yet."}</p>
                    </div>
                )}
            </div>
        </section>
